...
void ExtraAngVel::takeAction(std::shared_ptr<Agent> ag, const std::vector<int>& action) {
...
void ExtraAngVel3D::takeAction(std::shared_ptr<Agent> ag, const std::vector<int>& action) {
	auto itf = std::dynamic_pointer_cast<ChangeSwimVelAble>(ag);
	for (unsigned i = 0; i < ag->agentnum; i++) {
